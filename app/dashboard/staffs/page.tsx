...
export default function StaffPage() {
        ...
        try {
          ...
          if (res.ok) {
            ...
            if (data.status === 'SUCCESS') {
              ...
              setFpCommandId(null);
              showToast(data.message || 'Fingerprint rejected by device (fingerprint already in use or scan error).', 'error');
  ...
  const [logsFilter, setLogsFilter] = useState<'THIS_WEEK' | 'THIS_MONTH' | 'CUSTOM'>('THIS_MONTH');
  ...
  return (
    <div className="space-y-4">
                  ...
                  <div key={staff.id} className="p-4 rounded-2xl border border-slate-200 bg-white hover:border-slate-300 hover:shadow-md transition-all space-y-3">
                    ...
                    <div className="flex items-center justify-between pt-1">
                      <button
                        type="button"
                        onClick={() => {
                          setLogsFilter('THIS_MONTH');
                          setLogsCustomFrom('');
                          setLogsCustomTo('');
                          setExportTargetStaff(staff);
                          setShowExportModal(true);
                        }}
                        className="px-2.5 py-1.5 rounded-lg border border-slate-200 hover:bg-blue-50 hover:border-blue-300 text-slate-700 text-[11px] font-bold flex items-center gap-1 transition-all"
                        title="View Daily Work Hours & Logs"
                      >
      ...
      {/* ─── SLIDE-OVER PANEL: INDIVIDUAL STAFF DAILY LOGS ─── */}
      {showExportModal && exportTargetStaff && (() => {
        // Compute date-by-date breakdown for this staff
        const staffLogs = attendance.filter(r => r.staffId === exportTargetStaff.id);
        ...
        const filteredLogs = staffLogs.filter(r => {
          ...
          if (logsFilter === 'CUSTOM') {
            if (logsCustomFrom && d < logsCustomFrom) return false;
            if (logsCustomTo && d > logsCustomTo) return false;
            return true;
          }
          return true;
        });
        ...
        const totalMins = dateRows.reduce((acc, [, v]) => acc + v.mins, 0);
        ...
        return (
          <div className="fixed inset-0 !mt-0 z-50 flex" onClick={() => setShowExportModal(false)}>
            ...
            {/* Panel */}
            <div
              className="w-full sm:w-[420px] bg-white h-full flex flex-col shadow-2xl animate-in slide-in-from-right duration-300"
              onClick={e => e.stopPropagation()}
            >
              {/* Header */}
              <div className="p-5 border-b border-slate-200 flex items-start justify-between bg-gradient-to-br from-blue-900 to-slate-800 text-white">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-white/20 flex items-center justify-center font-black text-base">
                    {exportTargetStaff.name.charAt(0).toUpperCase()}
                  </div>
                  <div>
                    <h3 className="font-black text-base leading-tight">{exportTargetStaff.name}</h3>
                    <p className="text-xs text-blue-200 mt-0.5">{exportTargetStaff.role} · Daily Work Hours</p>
                  </div>
                </div>
                <button onClick={() => setShowExportModal(false)} className="text-white/70 hover:text-white mt-0.5">
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <div className="p-4 border-b border-slate-100 bg-slate-50 space-y-3">
                <div className="flex gap-1.5">
                  {(['THIS_WEEK', 'THIS_MONTH', 'CUSTOM'] as const).map(f => (
                    <button
                      key={f}
                      onClick={() => setLogsFilter(f)}
                      className={`flex-1 py-1.5 px-2 rounded-xl text-[11px] font-black transition-all border ${
                        logsFilter === f
                          ? 'bg-blue-900 text-white border-blue-900 shadow-sm'
                          : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-100'
                      }`}
                    >
                      {f === 'THIS_WEEK' ? 'This Week' : f === 'THIS_MONTH' ? 'This Month' : 'Custom'}
                    </button>
                  ))}
                </div>
                {logsFilter === 'CUSTOM' && (
                  <div className="flex items-center gap-2">
                    <div className="flex-1">
                      <label className="text-[10px] font-bold text-slate-500 uppercase mb-0.5 block">From</label>
                      <input
                        type="date"
                        value={logsCustomFrom}
                        onChange={e => setLogsCustomFrom(e.target.value)}
                        className="w-full px-2.5 py-1.5 border border-slate-200 rounded-lg text-xs font-semibold text-slate-700 outline-none focus:ring-2 focus:ring-blue-600 bg-white"
                      />
                    </div>
                    <div className="flex-1">
                      <label className="text-[10px] font-bold text-slate-500 uppercase mb-0.5 block">To</label>
                      <input
                        type="date"
                        value={logsCustomTo}
              ...
              {/* Total Hours KPI Banner */}
              <div className="px-4 py-3 bg-gradient-to-r from-purple-50 to-blue-50 border-b border-slate-100 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <Clock className="w-4 h-4 text-purple-700" />
                  <span className="text-xs font-bold text-slate-600">Total Hours ({dateRows.length} days)</span>
                </div>
                <div className="text-right">
                  <span className="text-lg font-black text-purple-900">{totalHrs}h {totalRem}m</span>
                  <p className="text-[10px] text-slate-400 font-semibold">{totalMins} minutes total</p>
                </div>
              </div>
              ...
              <div className="flex-1 overflow-y-auto">
                {dateRows.length === 0 ? (
                  <div className="flex flex-col items-center justify-center h-40 text-slate-400">
                    ...
                    <p className="text-xs text-slate-400 mt-1">Try a different filter range</p>
                  </div>
                ) : (
                  <div className="divide-y divide-slate-100">
                    {dateRows.map(([date, { mins, shifts }]) => {
                      const hrs = Math.floor(mins / 60);
                      const rem = mins % 60;
                      const isToday = date === todayStr;
                      const barPct = totalMins > 0 ? Math.round((mins / totalMins) * 100) : 0;
                      return (
                        <div key={date} className={`px-4 py-3 hover:bg-slate-50 transition-colors ${ isToday ? 'bg-blue-50/60' : '' }`}>
                          <div className="flex items-center justify-between mb-1.5">
                            <div className="flex items-center gap-2">
                              {isToday && (
                                <span className="text-[9px] font-black bg-blue-600 text-white px-1.5 py-0.5 rounded-full">TODAY</span>
                              )}
                              <span className="text-xs font-bold text-slate-800">{fmtDate(date)}</span>
                            </div>
                            <div className="text-right">
                              <span className="text-sm font-black text-purple-900">{hrs}h {rem}m</span>
                              <span className="text-[10px] text-slate-400 font-semibold ml-1.5">({shifts} shift{shifts > 1 ? 's' : ''})</span>
                            </div>
                          </div>
                          {/* Mini bar chart */}
              ...
              <div className="p-4 border-t border-slate-200 bg-slate-50 grid grid-cols-2 gap-2">
                <button
                  type="button"
                  onClick={() => exportIndividualAttendanceCSV(exportTargetStaff)}
                  className="py-2.5 px-3 bg-white hover:bg-slate-100 border border-slate-200 rounded-xl text-xs font-bold text-slate-800 flex items-center justify-center gap-1.5 transition-all shadow-xs"
                >
                  <FileSpreadsheet className="w-4 h-4 text-emerald-600" />
                  Download CSV
                </button>
                <button
                  type="button"
                  onClick={() => exportIndividualAttendancePDF(exportTargetStaff)}
                  className="py-2.5 px-3 bg-blue-50 hover:bg-blue-100 border border-blue-200 rounded-xl text-xs font-bold text-blue-900 flex items-center justify-center gap-1.5 transition-all shadow-xs"
                >
