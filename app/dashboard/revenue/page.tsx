...
export default function RevenuePage() {
  ...
  const [editUpiSenderName, setEditUpiSenderName] = useState('');
  ...
  const handleAddExpense = async (e: React.FormEvent) => {
    ...
    await addTransaction({
      ...
      upiId: expensePaymentMethod === 'UPI' ? expenseUpiId : null,
      upiSenderName: expensePaymentMethod === 'UPI' ? expenseUpiPayee : null
    });
  ...
  const handleEditTxInit = (tx: any) => {
    ...
    setEditUpiId(tx.upiId || '');
    setEditUpiSenderName(tx.upiSenderName || '');
  ...
  const handleEditTxSubmit = async (e: React.FormEvent) => {
    ...
    await updateTransaction(editingTxId, {
      ...
      upiId: editPaymentMethod === 'UPI' ? editUpiId : null,
      upiSenderName: editPaymentMethod === 'UPI' ? editUpiSenderName : null
    });
  ...
  const executeExport = () => {
    ...
    const rows = [
      ['Date (DD/MM/YYYY)', 'Time', 'Type', 'Category', 'Description', 'Payment Mode', 'UPI ID / Ref', 'UPI Payer/Payee', 'Amount (₹)', 'Paid Amount (₹)', 'Discount (₹)', 'Customer', 'Balance (₹)'],
  ...
  const executeExportPDF = async () => {
    ...
    const body = chronologicalTxs.map(t => {
      ...
      const paymentLabel = t.paymentMethod === 'UPI' && t.upiId 
        ? `UPI (${t.upiId})` 
        : (t.paymentMethod || 'CASH');

      return [
        t.time ? `${formatDateDDMMYYYY(t.date)}\n${t.time}` : formatDateDDMMYYYY(t.date),
        t.type,
        t.category,
        t.description?.replace(/"/g, "'") || '-',
        paymentLabel,
    ...
    autoTable(doc, {
      startY: 34,
      head: [['Date & Time', 'Type', 'Category', 'Description', 'Payment Mode', 'Amount', 'Customer', 'Balance']],
      body: body,
      theme: 'grid',
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-100/70 border-b border-slate-200 text-xs font-bold text-slate-600 uppercase tracking-wider">
                ...
                <th className="py-3 px-4">Mode & Details</th>
            ...
            <tbody className="divide-y divide-slate-200 text-xs sm:text-sm">
              {filteredTxs.map((tx) => (
                <tr key={tx.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="py-3 px-4">
                    <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-slate-100 text-slate-700 border border-slate-200 whitespace-nowrap inline-flex items-center gap-1">
                  ...
                  <td className="py-3 px-4">
                    <div className="text-slate-900 font-mono text-xs font-bold">{formatDateDDMMYYYY(tx.date)}</div>
                    {tx.time && <div className="text-slate-500 font-mono text-[11px] font-semibold">{tx.time}</div>}
                  </td>
                  <td
            ...
            <form onSubmit={handleAddExpense} className="space-y-4">
              ...
              <div>
                ...
                {expensePaymentMethod === 'UPI' && (
                  <div className="mt-3 p-3 bg-slate-50 border border-slate-200 rounded-xl space-y-3 animate-in fade-in">
                    <div>
                      <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">
                      ...
                      <input
                        type="text"
                        placeholder="e.g. 402819827361 or landlord@upi"
                        value={expenseUpiId}
                        onChange={(e) => setExpenseUpiId(e.target.value)}
                        className="w-full px-3 py-2 bg-white border border-slate-300 rounded-lg text-xs font-mono font-bold text-slate-900 outline-none focus:ring-2 focus:ring-rose-600"
                      />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">
                      ...
                      <input
                        ...
                        value={expenseUpiPayee}
                        onChange={(e) => setExpenseUpiPayee(e.target.value)}
                        className="w-full px-3 py-2 bg-white border border-slate-300 rounded-lg text-xs font-semibold text-slate-900 outline-none focus:ring-2 focus:ring-rose-600"
                      />
                    </div>
                  </div>
                )}
                  ...
                  <div className="mt-3 p-3 bg-slate-50 border border-slate-200 rounded-xl space-y-3 animate-in fade-in">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full px-3 py-2 bg-white border border-slate-300 rounded-lg text-xs font-mono font-bold text-slate-900 outline-none focus:ring-2 focus:ring-blue-900"
                      />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">
                      ...
                      <input
                        type="text"
                        placeholder="e.g. Sreeram / Vendor"
                        value={editUpiSenderName}
                        onChange={(e) => setEditUpiSenderName(e.target.value)}
                        className="w-full px-3 py-2 bg-white border border-slate-300 rounded-lg text-xs font-semibold text-slate-900 outline-none focus:ring-2 focus:ring-blue-900"
                      />
                    </div>
                  </div>
                )}
